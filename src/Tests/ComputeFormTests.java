...
public class ComputeFormTests {
    ...
    public void testComputeForm_SimpleFormula() {
        ...
        sheet.set(1, 0, "=A0*2");   // B0: formula depending on A0
        assertEquals("20.0", sheet.computeForm(1, 0)); // Expect "20" (10 * 2)
    }
    ...
    public void testComputeForm_MultipleDependencies() {
        ...
        sheet.set(2, 0, "=A0+B0");      // C0: depends on A0 and B0
        assertEquals("15.0", sheet.computeForm(2, 0)); // Expect "15" (5 + 10)
    }
    /// This one should stack over flow!!!)))
    @Test
    public void testComputeForm_CircularDependency() {
        ...
        assertEquals("ERROR: Circular dependency", sheet.computeForm(1, 0));
        assertEquals("ERROR: Circular dependency", sheet.computeForm(1, 0));
    }
    ...
    public void testComputeForm_DependencyOnText() {
        ...
        sheet.set(1, 0, "=A0+2");       // B0: tries to compute based on A0
    ...
    public void testComputeForm_SingleLevelDependency() {
        ...
        sheet.set(0, 0, "10");           // A0: 10
        sheet.set(1, 0, "=A0+5");      // B0: depends on A0
        assertEquals("15.0", sheet.computeForm(1, 0)); // Expect "15" (10 + 5)
    }
    ...
    public void testComputeForm_MultiLevelDependency() {
        ...
        sheet.set(0, 0, "5");             // A0: 5
        sheet.set(1, 0, "=A0*2");       // B0: depends on A0
        sheet.set(2, 0, "=B0+3");       // C0: depends on B0 (indirectly A0)
        assertEquals("13.0", sheet.computeForm(2, 0)); // Expect "13" (10 + 3)
    }
    ...
    public void testComputeForm_ComplexFormula() {
        ...
        sheet.set(0, 0, "5");             // A0: 5
        sheet.set(1, 0, "3");             // B0: 3
        sheet.set(2, 0, "=(A0+B0)*4"); // C0: complex formula
        assertEquals("32.0", sheet.computeForm(2, 0)); // Expect "32" ((5 + 3) * 4)
    }
}